...
// Listar sesiones (opcionalmente por status)
...
router.post("/", async (req, res) => {
  ...
  emitToDashboards("session:created", created); // ✅ ya trae createdBy.username
  res.json(created);
});
...
router.patch("/:id", async (req, res) => {
  ...
  emitToDashboards("session:updated", updated); // ✅ con createdBy.username
  res.json(updated);
});

// Confirmar salida (EMPLOYEE o ADMIN)
router.post("/:id/confirm-exit", async (req, res) => {
  ...
  const existing = await prisma.session.findUnique({ where: { id } });
  if (!existing) return res.status(404).json({ error: "No existe" });
  if (existing.status === "CONFIRMED_EXIT") {
    const same = await prisma.session.findUnique({ where: { id }, include: includeCreatedBy }); // ✅
    return res.json(same);
  }

  const updated = await prisma.session.update({
    where: { id },
    data: { status: "CONFIRMED_EXIT", confirmedExitById: user.id },
    include: includeCreatedBy, // ✅
  });
  ...
  emitToDashboards("session:statusChanged", updated); // ✅ con createdBy.username
  res.json(updated);
});

// Eliminar sesión (ADMIN o EMPLOYEE)
router.delete("/:id", async (req, res) => {
  ...
  // Permite a ADMIN y EMPLOYEE borrar cualquier registro
  if (user.role !== "ADMIN" && user.role !== "EMPLOYEE") {
    return res.status(403).json({ error: "Forbidden" });
  }

  await prisma.session.delete({ where: { id: existing.id } });
  emitToDashboards("session:deleted", { id: existing.id }); // solo id para borrar
